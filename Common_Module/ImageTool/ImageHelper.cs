...
namespace Common_Module.ImageTool
{
    public class ImageHelper
    {
        ...
        /// <summary>
        /// 将大图片分割成多个小图片，并返回小图片名称列表
        /// 于挺
        /// 日期：2017-11-14 18:07:52
        /// </summary>
        ...
        public static List<string> CutImageForList(Stream fromFile, string savepath, int smallimagewidth, int smallimageheight)
        {
            List<string> smallimage_list = new List<string>();
            Image sourceimage = Image.FromStream(fromFile);

            int width2 = sourceimage.Width;
            int height2 = sourceimage.Height;

            Bitmap newBitmap = new Bitmap(smallimagewidth, smallimageheight);
            Bitmap oldBitmap = (Bitmap)sourceimage;

            Color pixel;

            for (int i = 0; i < width2 / smallimagewidth; i++)
            {
                for (int j = 0; j < height2 / smallimageheight; j++)
                {
                    for (int p = i * smallimagewidth; p < i * smallimagewidth + smallimagewidth; p++)
                    {
                        for (int q = i * smallimageheight; q < j * smallimageheight + smallimageheight; q++)
                        {
                            pixel = oldBitmap.GetPixel(p, q);
                            newBitmap.SetPixel(p % smallimagewidth, q % smallimageheight, pixel);
                        }
                        string newimagefilename = string.Format("{0}_{1}_{2}.png", Guid.NewGuid().ToString("N"), i, j);
                        string newimagefilesavepath = savepath + newimagefilename;
                        try
                        {
                            newBitmap.Save(newimagefilesavepath, ImageFormat.Png);
                            smallimage_list.Add(newimagefilename);
                        }
                        catch (Exception ex)
                        {
                            throw ex;
