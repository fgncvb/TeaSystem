    ...
    public class Blowfish
    {
        ...
        public Blowfish(String password)
        {
            ...
            try
            {
                ...
                digest.ComputeHash(System.Text.Encoding.ASCII.GetBytes(password));
            }
            catch
            {
            }
        ...
        public String EncryptString(String sPlainText)
        {
            ...
            lock (m_rndGen)
            {
                ...
                lCBCIV = Blowfish.ByteArrayToLong(bb, 0);
            }
        ...
        private String EncStr(String sPlainText, long lNewCBCIV)
        {
            ...
            byte[] newCBCIV = new byte[Blowfish.BlowfishECB.BLOCKSIZE];
            LongToByteArray(lNewCBCIV, newCBCIV, 0);

            return BytesToBinHex(newCBCIV, 0, Blowfish.BlowfishECB.BLOCKSIZE)
        ...
        public String DecryptString(String sCipherText)
        {
            ...
            if (nLen < Blowfish.BlowfishECB.BLOCKSIZE)
                return null;

            // get the CBC IV
            byte[] cbciv = new byte[Blowfish.BlowfishECB.BLOCKSIZE];
            int nNumOfBytes = BinHexToBytes(sCipherText, cbciv, 0, 0,
                    Blowfish.BlowfishECB.BLOCKSIZE);
            if (nNumOfBytes < Blowfish.BlowfishECB.BLOCKSIZE)
                return null;

            // something left to decrypt?
            nLen -= Blowfish.BlowfishECB.BLOCKSIZE;
            if (nLen == 0)
            {
                return "";
            }
            ...
            nNumOfBytes = BinHexToBytes(sCipherText, buf,
                    Blowfish.BlowfishECB.BLOCKSIZE * 2, 0, nLen);
        ...
        private class BlowfishECB
        {
            ...
            public virtual void Encrypt(byte[] inbuffer, byte[] outbuffer)
            {
                ...
                for (int nI = 0; nI < nLen; nI += 8)
                {
                    ...
                    lTemp = Blowfish.ByteArrayToLong(inbuffer, nI);
            ...
            public virtual void Encrypt(byte[] buffer)
            {
                ...
                {
                    // encrypt a temporary 64bit block
                    lTemp = Blowfish.ByteArrayToLong(buffer, nI);
                    lTemp = EncryptBlock(lTemp);
            ...
            public virtual void Encrypt(int[] inbuffer, int[] outbuffer)
            {
                ...
                {
                    // encrypt a temporary 64bit block
                    lTemp = Blowfish.IntArrayToLong(inbuffer, nI);
                    lTemp = EncryptBlock(lTemp);
            ...
            public virtual void Encrypt(int[] buffer)
            {
                ...
                {
                    // encrypt a temporary 64bit block
                    lTemp = Blowfish.IntArrayToLong(buffer, nI);
                    lTemp = EncryptBlock(lTemp);
