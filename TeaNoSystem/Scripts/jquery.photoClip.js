(function (root, factory) { "use strict"; if (typeof define === "function" && define.amd) { define(["jquery", "iscroll-zoom", "hammer", "lrz"], factory); } else if (typeof exports === "object") { module.exports = factory(require("jquery"), require("iscroll-zoom"), require("hammer"), require("lrz")); } else { factory(root.jQuery, root.IScroll, root.Hammer, root.lrz); } }(this, function ($, IScroll, Hammer, lrz) {
    "use strict"; $.fn.photoClip = function (option) {
        if (!window.FileReader) { alert("您的浏览器不支持 HTML5 的 FileReader API， 因此无法初始化图片裁剪插件，请更换最新的浏览器！"); return; }
        var defaultOption = { size: [260, 260], outputSize: [0, 0], file: "", view: "", ok: "", rotaBtn: "", bigBtn: "", smallBtn: "", loadStart: function () { }, loadComplete: function () { }, loadError: function () { }, clipFinish: function () { } }
        $.extend(defaultOption, option); this.each(function () { photoClip(this, defaultOption); }); return this;
    }
    function photoClip(container, option) {
        var size = option.size, outputSize = option.outputSize, file = option.file, view = option.view, ok = option.ok, rotaBtn = option.rotaBtn, bigBtn = option.bigBtn, smallBtn = option.smallBtn, outputType = "image/jpeg", loadStart = option.loadStart, loadComplete = option.loadComplete, loadError = option.loadError, clipFinish = option.clipFinish; if (!isArray(size)) { size = [260, 260]; }
        if (!isArray(outputSize)) { outputSize = [0, 0]; }
        var clipWidth = size[0] || 260, clipHeight = size[1] || 260, outputWidth = Math.max(outputSize[0], 0), outputHeight = Math.max(outputSize[1], 0); var $file = $(file); if (!$file.length) return; var $img, imgWidth, imgHeight, imgLoaded; $file.attr("accept", "image/*"); $file.change(function () { if (!this.files.length) return; var files = this.files[0]; if (!/image\/\w+/.test(files.type)) { alert("图片格式不正确，请选择正确格式的图片文件！"); return false; } else { var fileReader = new FileReader(); fileReader.onprogress = function (e) { console.log((e.loaded / e.total * 100).toFixed() + "%"); }; fileReader.onload = function (e) { lrz(files).then(function (rst) { createImg(rst.base64); }).catch(function (err) { alert("图片处理失败"); loadError.call(this, err); }); }; fileReader.onerror = function (e) { alert("图片加载失败"); loadError.call(this, e); }; fileReader.readAsDataURL(files); loadStart.call(fileReader, files); } }); $file.click(function () { this.value = ""; }); var $container, $clipView, $moveLayer, $rotateLayer, $view, canvas, myScroll, containerWidth, containerHeight; init(); initScroll(); initEvent(); initClip();
        var $ok = $(ok);
        if ($ok.length)
        { $ok.click(function () { clipImg(); }); }
        ...
        $(bigBtn).click(function () {
            ...
            var sf = zoomNum * .25; myScroll.zoom(sf);

        });
        $(smallBtn).click(function () {
            if (zoomNum > 1) { zoomNum--; }


            var sf = zoomNum * .25; myScroll.zoom(sf);
        }); function initScroll() {
            var options = { zoom: true, scrollX: true, scrollY: true, freeScroll: true, mouseWheel: true, wheelAction: "zoom" }
        ...
        function initEvent() {
            var is_mobile = !!navigator.userAgent.match(/mobile/i); if (is_mobile) {
                var hammerManager = new Hammer($moveLayer[0]); hammerManager.add(new Hammer.Rotate()); var rotation, rotateDirection; hammerManager.on("rotatemove", function (e) {
                    if (atRotation) return; rotation = e.rotation; if (rotation > 180) { rotation -= 360; } else if (rotation < -180) { rotation += 360; }
                    rotateDirection = rotation > 0 ? 1 : rotation < 0 ? -1 : 0;
                }); hammerManager.on("rotateend", function (e) { if (atRotation) return; if (Math.abs(rotation) > 30) { if (rotateDirection == 1) { rotateCW(e.center); } else if (rotateDirection == -1) { rotateCCW(e.center); } } });
            } else {
                $moveLayer.on("dblclick", function (e) { rotateCW({ x: e.clientX, y: e.clientY }); });
        ...
        function rotateBy(angle, point) {
            ...
            var origin = calculateOrigin(curAngle, loacl),
                originX = origin.x, originY = origin.y, offsetX = 0, offsetY = 0, parentOffsetX = 0, parentOffsetY = 0, newAngle = curAngle + angle, curImgWidth, curImgHeight;
            if (curAngle == 0) {

                curX = 0; curY = 0;
            } else if (curAngle == 90 || curAngle == -270) {

                curX -= originX + originY; curY -= originY - originX;
            } else if (curAngle == 180 || curAngle == -180) {

                curX -= originX * 2; curY -= originY * 2;
            } else if (curAngle == 270 || curAngle == -90) {

                curX -= originX - originY; curY -= originX + originY;
            }
            if (newAngle == 90 || newAngle == -270) {

                offsetX = originX + originY; offsetY = originY - originX;
                if (newAngle > curAngle) {
                    parentOffsetX = imgHeight - originX - originY; parentOffsetY = originX - originY;
                } else if (newAngle < curAngle) {
                    parentOffsetX = (imgHeight - originY) - (imgWidth - originX); parentOffsetY = originX + originY - imgHeight;
                }
                curImgWidth = imgHeight;
                curImgHeight = imgWidth;
            } else if (newAngle == 180 || newAngle == -180) {

                offsetX = originX * 2; offsetY = originY * 2;
                if (newAngle > curAngle) {
                    parentOffsetX = (imgWidth - originX) - (imgHeight - originY); parentOffsetY = imgHeight - (originX + originY);
                } else if (newAngle < curAngle) {
                    parentOffsetX = imgWidth - (originX + originY); parentOffsetY = (imgHeight - originY) - (imgWidth - originX);
                }
                curImgWidth = imgWidth;
                curImgHeight = imgHeight;
            } else if (newAngle == 270 || newAngle == -90) {

                offsetX = originX - originY; offsetY = originX + originY;
                if (newAngle > curAngle) {
                    parentOffsetX = originX + originY - imgWidth; parentOffsetY = (imgWidth - originX) - (imgHeight - originY);
                } else if (newAngle < curAngle) {
                    parentOffsetX = originY - originX; parentOffsetY = imgWidth - originX - originY;
                } curImgWidth = imgHeight; curImgHeight = imgWidth;
            } else if (newAngle == 0 || newAngle == 360 || newAngle == -360) {
                offsetX = 0; offsetY = 0;
                if (newAngle > curAngle) {
                    parentOffsetX = originX - originY; parentOffsetY = originX + originY - imgWidth;
                } else if (newAngle < curAngle) {
                    parentOffsetX = originX + originY - imgHeight; parentOffsetY = originY - originX;
                }
                curImgWidth = imgWidth; curImgHeight = imgHeight;
            }


            curX = curX.toFixed(2) - 0; curY = curY.toFixed(2) - 0;
            setTransform($rotateLayer, curX, curY, curAngle, originX, originY);
            setTransition($rotateLayer, curX, curY, newAngle, 50, function () {
                atRotation = false; curAngle = newAngle % 360; curX += offsetX + parentOffsetX; curY += offsetY + parentOffsetY; curX = curX.toFixed(2) - 0; curY = curY.toFixed(2) - 0;
                ...
                if (myScroll.scale < myScroll.options.zoomMin) { myScroll.zoom(myScroll.options.zoomMin); }
        ...
        function clipImg() {
            ...
            ctx.save();
            if (!outputWidth || !outputHeight) { canvas.width = clipWidth / scale; canvas.height = clipHeight / scale; } else { canvas.width = outputWidth; canvas.height = outputHeight; ctx.scale(outputWidth / clipWidth * scale, outputHeight / clipHeight * scale); }
            ctx.translate(curX - local.x / scale, curY - local.y / scale); ctx.rotate(curAngle * Math.PI / 180); ctx.drawImage($img[0], 0, 0); ctx.restore();
            var dataURL = canvas.toDataURL(outputType, 1);
            $view.css("background-image", "url(" + dataURL + ")");
            clipFinish.call($img[0], dataURL);
        }

        var pattern, transforms = 0

        var monitor = setInterval(function () {
            pattern = new RegExp("\\((.| )+?\\)", "igm");
            if (transforms.toString() == $('.photo-clip-moveLayer').css('transform').match(pattern).toString()) {

            } else {
                // console.log('移动')
                clipImg();
            }
            transforms = $('.photo-clip-moveLayer').css('transform').match(pattern);
        ...
        function createImg(src) {
            clearImg();
            $img = $("<img>").css({ "user-select": "none", "pointer-events": "none" }); $img.load(imgLoad); $img.attr("src", src);
        }
        function setTransform($obj, x, y, angle, originX, originY) {
            ...
            style[prefix + "transform-origin"] = originX + "px " + originY + "px"; $obj.css(style);
